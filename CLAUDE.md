...
**L (새 기능):**
```
spec-analyst 명세 작성 → ux-writer 텍스트 설계
→ ⏸️ 사용자 확인 ("이렇게 진행할까요?")
→ 구현 (데이터 + UI) → code-reviewer 리뷰 → 빌드 확인
```

**XL (시스템 변경):**
```
spec-analyst 명세 작성 → ux-writer 텍스트 설계
→ ⏸️ 사용자 확인 (설계 + 텍스트 + 비용 영향)
→ 구현 → code-reviewer 리뷰 → 빌드 확인
```
...
2. **설계 확인은 L/XL만**: S/M은 바로 구현, L/XL은 설계 후 사용자 확인 한 번만 거침
3. **code-reviewer는 항상 실행**: 모든 규모에서 구현 후 자동 리뷰
4. **UX 텍스트 우선**: M 이상은 구현 전에 ux-writer가 텍스트를 먼저 확정
5. **사용자가 "바로 해줘"라고 하면**: 파이프라인을 축소하고 즉시 구현 (리뷰만 유지)

### 에이전트 목록

| 에이전트 | 파일 | 자동 호출 시점 |
|---------|------|-------------|
| `spec-analyst` | `.claude/agents/spec-analyst.md` | L/XL 기능 요청 시 |
| `ux-writer` | `.claude/agents/ux-writer.md` | M 이상 기능 요청 시 + 구현 후 텍스트 검증 |
| `code-reviewer` | `.claude/agents/code-reviewer.md` | 모든 코드 변경 후 |

### 산출물 체인

각 에이전트의 출력이 다음 에이전트의 입력이 된다:
```
spec-analyst → 요구사항 명세서 → ux-writer → 텍스트 확정안 → 구현 → code-reviewer → 리뷰 결과
```
