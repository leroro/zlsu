...
### 2. 회칙 (가입 후 회원용)
- **파일**: `public/rules/v2.1.md` (최신 버전, 항상 `RULES_VERSIONS[0]`)
...
## docs 폴더 구조 및 backlog 운영 규칙

### 구조

```
docs/
├── _backlog/                ← 개발 로드맵 (항상 맨 위)
│   ├── 1xx_xxx/             ← 프론트엔드 기능
│   ├── 5xx_xxx/             ← 백엔드/인프라 (프론트 완료 후)
│   └── archive/             ← 완료/흡수된 항목
├── 00_README.md             ← docs 안내 + 정리 규칙
├── 01_배포_가이드.md          ← 현재 유효한 참고 문서
├── 02_회원가입_프로세스.md     ← 현재 유효한 참고 문서
└── YYYY-MM-DD/              ← 세션별 작업 아카이브
```

### backlog 넘버링 규칙

- **형식**: `{3자리번호}_{이름}/` (폴더)
- **첫 자리 = 카테고리**: 1 = 프론트엔드, 5 = 백엔드/인프라
- **뒤 두 자리 = 순번**: 카테고리 내 실행 순서
- **새 기능 추가**: 해당 카테고리 다음 순번 폴더 생성 (예: `102_새기능/`)
- **완료 시**: `archive/`로 이동
- **세션 시작 시**: `_backlog/` 폴더를 확인하여 현재 진행 중인 항목 파악
- **세션 종료 시**: 진행 상황 업데이트, 새 항목 추가, 완료 항목 아카이브
...
## 에이전트 오케스트레이션 (자동 워크플로우)

**사용자가 기능 요청을 하면, 작업 규모를 판단하여 아래 파이프라인을 자동 실행한다.**
사용자가 특정 에이전트를 지정하지 않는 한, 메인 Claude가 오케스트레이터 역할을 수행한다.

### 핵심 원칙: 가성비 최적화

**비용 최소화가 아니라, 작업 가치에 맞는 적정 투자를 한다.**
- 간단한 건 빠르게 → 풀 파이프라인은 오히려 낭비
- 중요한 건 제대로 → 스킵했다가 재작업하면 더 비쌈
- 리소스 = 토큰 사용량 + 시간 + 품질의 밸런스

### 작업 규모 판단 기준

| 규모 | 판단 기준 | 예시 |
|------|----------|------|
| **S** | 텍스트·스타일만 변경, 1~2파일 | 오타 수정, 색상 변경, 문구 수정 |
| **M** | 기존 화면에 요소 추가/수정, 3~5파일 | 버튼 추가, 섹션 추가, 라벨 변경 |
| **L** | 새 기능·새 페이지, 5파일 이상 | 환영 메시지, 새 관리 페이지 |
| **XL** | 시스템 구조 변경 | 이중 승인, 결제 연동, DB 마이그레이션 |

### 규모별 파이프라인

**S (단순 수정) — 저비용, 확인 없음:**
```
구현 → code-reviewer 리뷰 → 빌드 확인
```

**M (기능 개선) — 저비용, 확인 없음:**
```
ux-writer 텍스트 검토 → 구현 → code-reviewer 리뷰 → 빌드 확인
```

**L (새 기능) — 중비용, 설계 확인 1회:**
```
spec-analyst 명세 → ux-writer 텍스트 설계
→ ⏸️ 사용자 확인 (진행 방식 선택)
→ 구현 (data-developer + ui-developer) → code-reviewer 리뷰 → 빌드 확인
```

**XL (시스템 변경) — 고비용, 범위 확인 + 설계 확인:**
```
⏸️ 사용자에게 범위/투자 수준 확인
→ product-owner 방향성 검토 → spec-analyst 명세 → ux-writer 텍스트 설계
→ ⏸️ 사용자 확인 (설계 + 비용 영향)
→ 구현 → code-reviewer 리뷰 → qa-tester 검증 → 빌드 확인
```

### L/XL에서 사용자에게 보여주는 형식
...
| 사용자 말 | 효과 |
|----------|------|
| "바로 해" / "간단하게" | 파이프라인 다운그레이드 (L→M, M→S) |
| "중요해" / "신경 써" | 파이프라인 업그레이드 (M→L), 리뷰 강화 |
| 별말 없음 | 규모별 기본 파이프라인 |

### 실행 규칙

1. **규모 판단은 자동**: 사용자에게 "이건 M 규모입니다" 식으로 물어보지 않고 바로 적절한 파이프라인 실행
2. **S/M은 확인 없이 바로 실행**: 자잘한 것까지 매번 묻지 않는다
3. **L은 설계 확인 1회**: "이렇게 진행할까요?" + 간소화 옵션 제시
4. **XL은 범위 확인부터**: 먼저 "이 정도 투자가 적절한데, 이대로 할까요?" 확인
5. **code-reviewer는 항상 실행**: 모든 규모에서 구현 후 자동 리뷰
6. **UX 텍스트 우선**: M 이상은 구현 전에 ux-writer가 텍스트를 먼저 확정

### 실행 로그

파이프라인 실행 시 각 단계를 기록한다. 사용자가 "왜 오래 걸려?" 등 질문하면 이 로그를 참조하여 병목 분석.
- 세션 종료 시 cost-advisor가 실행 로그 분석 → agent-memory에 피드백 축적
- 반복되는 비효율 패턴 감지 시 파이프라인 규칙 자동 조정 제안

### 에이전트 목록

| 에이전트 | 모델 | 자동 호출 시점 |
|---------|------|-------------|
| `product-owner` | Opus | XL 기능: 방향성/정책 검토 |
| `spec-analyst` | Opus | L/XL 기능: 요구사항 분석 + 시스템 설계 |
| `ux-writer` | Opus | M 이상: 텍스트 설계 + 구현 후 정합성 검증 |
| `ui-developer` | Opus | L/XL: 화면 구현 |
| `data-developer` | Opus | L/XL: 타입/API/데이터 구현 |
| `code-reviewer` | Sonnet | 항상: 코드 변경 후 리뷰 + 시스템 일관성 검증 |
| `qa-tester` | Sonnet | XL: 테스트 시나리오 + 검증 |
| `devops` | Haiku | 배포/세션 종료 시: 빌드/배포/커밋 |
| `cost-advisor` | Sonnet | XL + 세션 종료: 비용 영향 분석 + 실행 로그 리뷰 |

### 산출물 체인

각 에이전트의 출력이 다음 에이전트의 입력이 된다:
```
[XL] product-owner → 방향성
[L+] spec-analyst → 요구사항 명세서
[M+] ux-writer → 텍스트 확정안
[구현] data-developer + ui-developer → 코드
[항상] code-reviewer → 리뷰 결과
[XL] qa-tester → 검증 결과
```
